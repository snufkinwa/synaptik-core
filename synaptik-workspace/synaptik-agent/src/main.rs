...
#[derive(Parser, Debug)]
#[command(name = "synaptik-agent", about = "Synaptik contracts auto-updater agent")]
struct Cli {
...
#[derive(Debug, Clone, Deserialize)]
struct TrustKey { key_id: String, pubkey_hex: String, #[serde(default)] not_before: Option<String>, #[serde(default)] not_after: Option<String> }

#[derive(Debug, Clone, Deserialize)]
struct TrustStore { #[serde(default)] keys: Vec<TrustKey> }

#[derive(Debug, Clone, Deserialize)]
struct RegistryEvent {
    #[allow(dead_code)] t: Option<String>,
    op: String,
    #[serde(default)] channel: Option<String>,
    #[serde(default)] uri: Option<String>,
...
fn run_once(cfg: &AgentConfig) -> Result<()> {
    let (pack_path, revoked_keys, revoked_root_uuids) = resolve_pack_uri(&cfg.registry_uri, cfg.channel.as_deref())?
    ...
    for root in &cfg.roots {
        ...
        if revoked_root_uuids.contains(&ruuid) { emit_status(&root_path, &pack.version, "REVOKED_ROOT", false, None, 0, cfg.channel.as_deref()); continue; }
...
fn resolve_pack_uri(registry_uri: &str, channel: Option<&str>) -> Result<Option<(String, HashSet<String>, HashSet<String>)>> {
    // If registry_uri points directly to a JSON file that looks like a pack, return it.
    if registry_uri.ends_with(".json") && Path::new(registry_uri.trim_start_matches("file://")).exists() {
        return Ok(Some((registry_uri.trim_start_matches("file://").to_string(), HashSet::new(), HashSet::new())));
    }
    ...
    for line in text.lines() {
        ...
        if let Ok(ev) = serde_json::from_str::<RegistryEvent>(line) { events.push(ev); }
    }
        ...
        match ev.op.as_str() {
            "PUBLISH" => {
                if let (Some(ch), Some(uri)) = (ev.channel.as_ref(), ev.uri.as_ref()) {
                    ...
                    if let Some(ver) = ev.version.as_ref() { version_uri.insert(ver.clone(), uri.clone()); }
                }
            }
            "PROMOTE" => {
            ...
            "REVOKE_KEY" => {
                if let Some(k) = ev.key_id.as_ref() { revoked_keys.insert(k.clone()); }
            }
            "REVOKE_ROOT" => { if let Some(u) = ev.uuid.as_ref() { revoked_roots.insert(u.clone()); } }
            "DEPRECATE_VERSION" => {
                if let Some(v) = ev.version.as_ref() { deprecated_versions.insert(v.clone()); }
            }
            _ => {}
        }
    }
    ...
    if let Some((uri, ver)) = chan_uri.get(ch).cloned() {
        if let Some(v) = ver {
            if deprecated_versions.contains(&v) { return Ok(None); }
        }
        return Ok(Some((uri.trim_start_matches("file://").to_string(), revoked_keys, revoked_roots)));
    }
    Ok(None)
}
...
fn verify_pack(pack: &ContractPack, cfg: &AgentConfig, revoked: &HashSet<String>) -> Result<()> {
    ...
    if let Some(sig_b64) = &pack.signature {
        ...
        let sig_bytes = B64.decode(sig_b64.as_bytes()).context("decode signature b64")?;
        ...
        if let Some(store_path) = cfg.trust_store.as_deref() {
            if Path::new(store_path).exists() {
                ...
                let store: TrustStore = toml::from_str(&String::from_utf8_lossy(&tbytes)).context("parse trust_store")?;
                let kid_filter = pack.signing_key_id.as_deref();
                for k in store.keys {
                    // key id filter when present
                    if let Some(kid) = kid_filter { if k.key_id != kid { continue; } }
                    ...
                    if let Some(nb) = &k.not_before {
                        match chrono::DateTime::parse_from_rfc3339(nb) {
                            Ok(dt) => { if Utc::now() < dt.with_timezone(&Utc) { continue; } }
                            Err(_) => continue,
                        }
                    }
                    ...
                    let pk_vec = hex::decode(k.pubkey_hex.trim_start_matches("ed25519:")).context("decode pubkey hex")?;
                    let pk: [u8; 32] = pk_vec.try_into().map_err(|_| anyhow!("pubkey length != 32"))?;
        ...
        if vkeys.is_empty() {
            if let Some(pk_hex) = cfg.trusted_pubkey_hex.as_deref() {
                let pk_vec = hex::decode(pk_hex.trim_start_matches("ed25519:")).context("decode pubkey hex")?;
                let pk: [u8; 32] = pk_vec.try_into().map_err(|_| anyhow!("pubkey length != 32"))?;
                let vk = VerifyingKey::from_bytes(&pk).map_err(|e| anyhow!("pubkey: {e}"))?;
        ...
        if let Some(kid) = &pack.signing_key_id { anyhow::ensure!(!revoked.contains(kid), "signing key revoked: {kid}"); }

        let mut ok = false;
        for (_kid, vk) in vkeys {
            if vk.verify_strict(&msg, &sig).is_ok() { ok = true; break; }
        }
...
fn process_root(root: &Path, pack: &ContractPack, agent_cfg: &AgentConfig, _revoked: &HashSet<String>, dry_run: bool) -> Result<()> {
    ...
    let incoming_b64 = pack.blobs.get(&rel)
        .ok_or_else(|| anyhow!("pack missing blob for {}", rel))?;
    let incoming_bytes = B64.decode(incoming_b64.as_bytes()).context("decode incoming blob")?;
    ...
    if agent_cfg.require_locked.unwrap_or(true) && !local_matches_embedded(root, &default_name) {
        emit_status(root, &pack.version, "LOCKED", had_conflicts, None, 0, agent_cfg.channel.as_deref());
    ...
    if had_conflicts || !parses {
        ...
        if let Some(parent) = conf_path.parent() { let _ = fs::create_dir_all(parent); }
        ...
        emit_status(root, &pack.version, if had_conflicts { "CONFLICT" } else { "PARSE_FAIL" }, had_conflicts, None, 0, agent_cfg.channel.as_deref());
        return Err(anyhow!("bind unsafe: conflicts={} parses={}", had_conflicts, parses));
    }

    if dry_run {
        emit_status(root, &pack.version, "NO_UPDATE", false, Some(blake3::hash(merged.as_bytes()).to_hex().to_string()), 0, agent_cfg.channel.as_deref());
        return Ok(());
    }

    // ATP metering (rough heuristic: per-line cost)
    let atp = merged.lines().count() as u64;
    if let Some(max) = agent_cfg.max_atp_per_apply { if atp > max { emit_status(root, &pack.version, "ATP_CAP", false, None, atp, agent_cfg.channel.as_deref()); return Err(anyhow!("apply exceeds ATP cap: {}>{}", atp, max)); } }
    ...
    if let Some(day_cap) = agent_cfg.max_atp_per_day {
        ...
        if state.used.saturating_add(atp) > day_cap {
            emit_status(root, &pack.version, "ATP_DAILY_CAP", false, None, atp, agent_cfg.channel.as_deref());
            return Err(anyhow!("daily ATP cap reached: {}+{}>{}", state.used, atp, day_cap));
        }
    ...
    let backup_path = root.join("contracts").join(format!(".backup-{}-{}.toml", default_name, Utc::now().format("%Y%m%dT%H%M%S")));
    if target_path.exists() { let _ = fs::copy(&target_path, &backup_path); }

    // Apply with RAII unlock → apply → relock
    cmds.unlock_contracts();
    let (_merged, _conf) = cmds.contracts_bind_with_text(&incoming_text, true, None).map_err(|e| {
        // rollback immediate
        if backup_path.exists() { let _ = fs::copy(&backup_path, &target_path); }
        e
    })?;
    cmds.lock_contracts();
    ...
    if let Some(parent) = rec_path.parent() { let _ = fs::create_dir_all(parent); }
    let _ = fs::write(&rec_path, serde_json::to_vec_pretty(&receipt)?);
    emit_status(root, &pack.version, "APPLIED", false, Some(mhash), atp, agent_cfg.channel.as_deref());
    Ok(())
}

fn normalize_toml(raw: &str) -> String {
    raw.parse::<DocumentMut>().map(|d| d.to_string()).unwrap_or_else(|_| raw.to_string())
}
...
fn local_matches_embedded(root: &Path, default_name: &str) -> bool {
    ...
    let Ok(bytes) = fs::read(&local) else { return false; };
...
fn emit_status(root: &Path, pack_version: &str, status: &str, had_conflicts: bool, merged_hash: Option<String>, atp_cost: u64, channel: Option<&str>) {
...
struct AtpState { day: String, used: u64 }

fn atp_state_path(root: &Path) -> PathBuf { root.join("contracts/.atp.json") }
