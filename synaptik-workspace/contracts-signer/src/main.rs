...
#[derive(Parser, Debug)]
#[command(
    name = "contracts-signer",
    about = "Build and sign a Synaptik contracts pack"
)]
struct Cli {
...
fn main() -> Result<()> {
    ...
    let list: Vec<PackFileEntry> = files
        .iter()
        .map(|(rel, bytes)| PackFileEntry {
            path: format!("contracts/{rel}"),
    ...
    if let Some(skhex) = cli.sk_hex.as_deref() {
        ...
        let sk =
            SigningKey::from_bytes(&sk_bytes.try_into().map_err(|_| anyhow!("bad sk length"))?);
    ...
    fs::write(&cli.out, serde_json::to_vec_pretty(&pack)?)
        .with_context(|| format!("write {}", &cli.out))?;
    Ok(())
}

// Normalize a relative path into a forward-slash separated UTF-8 string.
// This prevents Windows manifests from containing backslashes, ensuring
// canonical pack entries across platforms.
fn normalize_rel(p: &Path) -> String {
    // Prefer component join for stricter normalization; fallback to lossily decoded string.
    let s = p
        .components()
        .map(|c| c.as_os_str().to_string_lossy())
        .collect::<Vec<_>>()
        .join("/");
    // In case the path had a prefix or something unusual, also replace any stray backslashes.
    s.replace('\\', "/")
}
