...
use contracts::normalize; // bring normalize::for_rules into scope for test helper
...
fn zero_width_obfuscated_sequence_is_fully_masked() {
    ...
    let masked = {
        ...
        for pat in patterns.iter() {
            ...
            let plen = pchars.len();
            if plen == 0 || plen > nchars.len() {
                continue;
            }
            let mut i = 0;
            let mut ranges = Vec::<(usize, usize)>::new();
            while i + plen <= nchars.len() {
                if (0..plen).all(|j| nchars[i + j] == pchars[j]) {
                    let (s, _) = spans[i];
...
fn combining_marks_are_masked_as_whole() {
    ...
    fn norm_lower(s: &str) -> String {
        contracts::normalize::for_rules(s)
    }
    fn normalized_chars_with_spans(s: &str) -> (Vec<char>, Vec<(usize, usize)>) {
        let mut chars = Vec::new();
        let mut spans = Vec::new();
    ...
    assert!(
        !out.contains("ss keys"),
        "Partial leak after combining mark: {out}"
    );
}
