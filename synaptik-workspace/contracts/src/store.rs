...
impl ContractsStore {
    ...
    fn capsules_dir(&self) -> PathBuf { self.root.join("capsules") }
    fn ann_dir(&self) -> PathBuf { self.root.join("annotations") }
    fn handle_dir(&self) -> PathBuf { self.root.join("handles") }
    ...
    pub fn latest_annotation(&self, id: &CapsId) -> Result<Option<CapsAnnot>> {
        ...
        if !latest.exists() { return Ok(None); }
    ...
    pub fn gate_replay(&self, id: &CapsId, _purpose: Purpose) -> std::result::Result<(), Denied> {
        match self.latest_annotation(id).map_err(|e| Denied { reason: format!("store error: {e}"), verdict: Verdict::Quarantine, risk: 1.0, labels: vec!["store_error".into()] })? {
            Some(ann) => {
                // DEV_ALLOW TTL enforcement: if enabled and expired, deny.
                if ann.labels.iter().any(|l| l == "DEV_ALLOW") {
                    if let Some(ttl_ms) = std::env::var("COGNIV_DEV_AUTO_ALLOW_TTL_MS").ok().and_then(|s| s.parse::<u64>().ok()) {
                        let now_ms = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis() as u64;
                        if now_ms.saturating_sub(ann.ts_ms) > ttl_ms {
                            return Err(Denied { reason: "dev_allow_expired".into(), verdict: Verdict::Quarantine, risk: 1.0, labels: ann.labels });
                        }
                    }
                }
                match ann.verdict {
                    Verdict::Allow | Verdict::AllowWithPatch => Ok(()),
                    Verdict::Quarantine => Err(Denied { reason: "quarantined".into(), verdict: Verdict::Quarantine, risk: ann.risk, labels: ann.labels }),
                }
            }
            None => Err(Denied { reason: "annotation_pending".into(), verdict: Verdict::Quarantine, risk: 1.0, labels: vec!["pending".into()] }),
        }
    }
    ...
    pub fn map_memory(&self, memory_id: &str, caps_id: &CapsId) -> Result<()> {
        let p = self.handle_dir().join("memory").join(format!("{}.txt", sanitize(memory_id)));
...
fn maybe_dev_auto_allow(store: &ContractsStore, cap: &SimCapsule, id: &CapsId) -> Result<()> {
    // Enabled?
    let enabled = std::env::var("COGNIV_DEV_AUTO_ALLOW").ok().map(|v| v == "1" || v.eq_ignore_ascii_case("true")).unwrap_or(false);
    if !enabled { return Ok(()); }

    // Namespace filter
    let prefix = std::env::var("COGNIV_DEV_AUTO_ALLOW_PREFIX").unwrap_or_else(|_| "dev/".to_string());
    let lobe_ok = cap.meta.lobe.as_deref().map(|l| l.starts_with(&prefix)).unwrap_or(false);
    if !lobe_ok { return Ok(()); }

    // Size cap (bytes of outputs serialized)
    let max_bytes: usize = std::env::var("COGNIV_DEV_AUTO_ALLOW_MAX_BYTES").ok().and_then(|s| s.parse::<usize>().ok()).unwrap_or(64 * 1024);
    let out_bytes = serde_json::to_vec(&cap.outputs).unwrap_or_default();
    if out_bytes.len() > max_bytes { return Ok(()); }

    // Annotate immediately as Allow (dev) with label DEV_ALLOW
    let now_ms = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis() as u64;
    let annot = CapsAnnot { verdict: Verdict::Allow, risk: 0.0, labels: vec!["DEV_ALLOW".into()], policy_ver: "dev".into(), patch_id: None, ts_ms: now_ms };
    let _ = store.annotate(id, &annot);
    eprintln!("[contracts] DEV_ALLOW applied to capsule {} (lobe prefix {}), expires per TTL env", id, prefix);
    Ok(())
}
