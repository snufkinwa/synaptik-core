...
use crate::py_helpers::{
    gen_path_name, json_array_to_py, json_to_py, metadata_to_py, bind_json, object_ref_to_py, path_exists_in_refs,
    py_to_json, pyerr, sanitize_name,
};
...
impl PyCommands {
    ...
    fn new() -> PyResult<Self> {
        ...
        Ok(Self { inner, last_recalled: None })
    }
    ...
    fn govern_text(&self, py: Python<'_>, intent: &str, input: &str) -> PyResult<PyObject> {
        ...
        impl Iterator for EchoStream {
            type Item = String;
            fn next(&mut self) -> Option<Self::Item> { if self.yielded { None } else { self.yielded = true; Some(self.text.clone()) } }
        }
        struct EchoModel { text: String }
        impl LlmClient for EchoModel {
            type Stream = EchoStream;
            fn stream(&self, _system_prompt: String) -> Result<Self::Stream, syn_core::services::GateError> { Ok(EchoStream { yielded: false, text: self.text.clone() }) }
        }
    ...
    fn reflect(&self, lobe: &str, window: usize) -> PyResult<String> { self.inner.reflect(lobe, window).map_err(pyerr) }

    #[pyo3(signature = (lobe, n=10))]
    fn recent(&self, lobe: &str, n: usize) -> PyResult<Vec<String>> { self.inner.recent(lobe, n).map_err(pyerr) }

    #[pyo3(signature = (memory_id, prefer=None))]
    fn recall(&self, py: Python<'_>, memory_id: &str, prefer: Option<&str>) -> PyResult<Option<PyObject>> {
        let p = match prefer.unwrap_or("auto") { "hot" => Prefer::Hot, "archive" => Prefer::Archive, "dag" => Prefer::Dag, _ => Prefer::Auto };
    ...
    fn recall_prefer(&self, memory_id: &str, prefer: Option<&str>) -> PyResult<Option<String>> {
        let p = match prefer.unwrap_or("auto") { "hot" => Prefer::Hot, "archive" => Prefer::Archive, "dag" => Prefer::Dag, _ => Prefer::Auto };
        Ok(self.inner.recall_any(memory_id, p).map_err(pyerr)?.map(|r| r.content))
    }

    #[pyo3(signature = (memory_ids, prefer=None))]
    fn recall_many(&self, py: Python<'_>, memory_ids: Vec<String>, prefer: Option<&str>) -> PyResult<PyObject> {
        let p = match prefer.unwrap_or("auto") { "hot" => Prefer::Hot, "archive" => Prefer::Archive, "dag" => Prefer::Dag, _ => Prefer::Auto };
    ...
    fn pons_create(&self, name: &str) -> PyResult<()> { self.inner.pons_create(name).map_err(pyerr) }
    ...
    fn pons_put_object(
        ...
        let slice = unsafe { std::slice::from_raw_parts(mv.buf_ptr() as *const u8, mv.item_count()) };
        ...
        let obj = self.inner.pons_put_object(pons, key, &bytes, media_type, extra_v).map_err(pyerr)?;
        Ok(crate::py_helpers::json_to_py(py, &json!({
            "pons": obj.pons,
            "key": obj.key,
            "version": obj.version,
            "etag": obj.etag,
    ...
    fn pons_list_latest(&self, py: Python<'_>, pons: &str, prefix: Option<&str>, limit: usize) -> PyResult<Vec<PyObject>> {
        let refs = self.inner.pons_list_latest(pons, prefix, limit).map_err(pyerr)?;
        let mut out = Vec::with_capacity(refs.len());
        for r in refs { out.push(object_ref_to_py(py, &r)); }
        Ok(out)
    }
    ...
    fn stats(&self, py: Python<'_>, lobe: Option<&str>) -> PyResult<PyObject> {
        ...
        for (l, c) in s.by_lobe { by_lobe.append((l, c))?; }
    ...
    fn recall_snapshot(&mut self, py: Python<'_>, snapshot_id: &str, export: Option<&str>) -> PyResult<PyObject> {
        let s = self.inner.replay_recall_snapshot(snapshot_id).map_err(pyerr)?;
        if let Some(path) = export { std::fs::write(path, &s.content).map_err(pyerr)?; }
    ...
    fn recall_and_diverge(&mut self, py: Python<'_>, snapshot_id: &str, path_name: Option<&str>, export: Option<&str>) -> PyResult<String> {
        let _ = self.recall_snapshot(py, snapshot_id, export)?;
        let name = path_name.map(sanitize_name).unwrap_or_else(|| gen_path_name("replay", snapshot_id));
        let id = self.inner.replay_diverge_from(snapshot_id, &name).map_err(pyerr)?;
        Ok(id)
    }
    ...
    fn diverge_from(&self, snapshot_id: &str, path_name: Option<&str>) -> PyResult<String> {
        let name = path_name.map(sanitize_name).unwrap_or_else(|| gen_path_name("replay", snapshot_id));
        self.inner.replay_diverge_from(snapshot_id, &name).map_err(pyerr)
    }

    #[pyo3(signature = (path_name, content, meta=None))]
    fn extend_path(&mut self, py: Python<'_>, path_name: &str, content: &str, meta: Option<PyObject>) -> PyResult<String> {
        let norm = sanitize_name(path_name);
        if !path_exists_in_refs(&norm).map_err(pyerr)? {
            let base = self.last_recalled.clone().ok_or_else(|| pyerr(anyhow!(format!("Path '{}' not found and no prior recall to seed from.", path_name))))?;
            let _ = self.inner.replay_diverge_from(&base, &norm).map_err(pyerr)?;
        }
        let mut meta_value: Value = if let Some(obj) = meta { let bound = obj.bind(py); py_to_json(&bound) } else { json!({}) };
    ...
    meta_value = bind_json(enrich, meta_value);
        let state = DagMemoryState { content: content.to_string(), meta: meta_value };
        let new_id = self.inner.replay_extend_path(&norm, state).map_err(pyerr)?;
        Ok(new_id)
    }

    fn snapshot_meta(&self, py: Python<'_>, snapshot_id: &str) -> PyResult<PyObject> { Ok(json_to_py(py, &self.inner.dag_snapshot_meta(snapshot_id).map_err(pyerr)?)) }
    ...
    fn last_recalled_id(&self) -> Option<String> { self.last_recalled.clone() }

    fn latest_on_path(&self, path_name: &str) -> PyResult<Option<String>> {
        let norm = sanitize_name(path_name);
        let v = self.inner.dag_trace_path(&norm, 1).map_err(pyerr)?;
        if let Some(first) = v.first() { if let Some(hash) = first.get("hash").and_then(|x| x.as_str()) { return Ok(Some(hash.to_string())); } }
        Ok(None)
    }

    #[pyo3(signature = (path_name, export=None))]
    fn recall_latest_on_path(&mut self, py: Python<'_>, path_name: &str, export: Option<&str>) -> PyResult<Option<PyObject>> {
        match self.latest_on_path(&sanitize_name(path_name))? {
            Some(hash) => { let obj = self.recall_snapshot(py, &hash, export)?; Ok(Some(obj)) }
            None => Ok(None),
        }
    }
    ...
    fn seed_base_from_lobe(&self, lobe: &str) -> PyResult<Option<String>> { self.inner.replay_base_from_lobe(lobe).map_err(pyerr) }

    #[pyo3(signature = (lobe, path_name=None))]
    fn begin_branch(&mut self, lobe: &str, path_name: Option<&str>) -> PyResult<String> {
        let cid = self.inner.replay_base_from_lobe(lobe).map_err(pyerr)?.ok_or_else(|| pyerr(anyhow!(format!("No base snapshot available for lobe '{lobe}'"))))?;
        let name = path_name.map(sanitize_name).unwrap_or_else(|| gen_path_name("replay", &cid));
    ...
    fn encode_engram(&self, py: Python<'_>, path: &str, content: &str, meta: Option<PyObject>) -> PyResult<String> {
        let norm = sanitize_name(path);
        let meta_value = meta.as_ref().map(|o| py_to_json(&o.bind(py))).unwrap_or(serde_json::json!({}));
        self.inner.append(&norm, content, Some(meta_value)).map_err(pyerr)
    }

    fn dag_head(&self, path_name: &str) -> PyResult<Option<String>> { self.inner.dag_head(&sanitize_name(path_name)).map_err(pyerr) }

    fn update_path_head(&self, path_name: &str, snapshot_hash: &str) -> PyResult<()> { self.inner.update_path_head(&sanitize_name(path_name), snapshot_hash).map_err(pyerr) }

    fn engram_head(&self, path_name: &str) -> PyResult<Option<String>> { self.inner.dag_head(&sanitize_name(path_name)).map_err(pyerr) }

    fn set_engram_head(&self, path_name: &str, snapshot_hash: &str) -> PyResult<()> { self.inner.update_path_head(&sanitize_name(path_name), snapshot_hash).map_err(pyerr) }
