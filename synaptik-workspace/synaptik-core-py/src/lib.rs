...
fn py_to_json(any: &Bound<'_, PyAny>) -> Value {
    ...
    if let Ok(f) = any.extract::<f64>() { return json!(f); }
...
impl PyCommands {
    ...
    fn extend_path(&mut self, py: Python<'_>, path_name: &str, content: &str, meta: Option<PyObject>) -> PyResult<String> {
        ...
        if !path_exists_in_refs(&norm).map_err(pyerr)? {
            let base = self.last_recalled.clone().ok_or_else(|| pyerr(anyhow!(
    ...
    fn begin_branch(&mut self, lobe: &str, path_name: Option<&str>) -> PyResult<String> {
        let cid = self
            ...
            .map_err(pyerr)?
            .ok_or_else(|| pyerr(anyhow!(format!("No base snapshot available for lobe '{lobe}'"))))?;
        let name = path_name
            .map(sanitize_name)
    ...
    fn append(&self, py: Python<'_>, path: &str, content: &str, meta: Option<PyObject>) -> PyResult<String> {
        let norm = sanitize_name(path);
        let meta_value = meta
            .as_ref()
    ...
    fn reconsolidate_paths(&self, src_path: &str, dst_path: &str, note: &str) -> PyResult<String> {
        ...
        self.inner.reconsolidate_paths(&dst, &src, note).map_err(pyerr)
    }

    /// Neuroscience alias: sprout a dendrite (normalized, idempotent branch).
    #[pyo3(signature = (path, base=None, lobe=None))]
    fn sprout_dendrite(&self, path: &str, base: Option<&str>, lobe: Option<&str>) -> PyResult<String> {
        let norm = sanitize_name(path);
        self.inner.branch(&norm, base, lobe).map_err(pyerr)
    }
    ...
    fn update_path_head(&self, path_name: &str, snapshot_hash: &str) -> PyResult<()> {
        let norm = sanitize_name(path_name);
        self.inner.update_path_head(&norm, snapshot_hash).map_err(pyerr)
    }
